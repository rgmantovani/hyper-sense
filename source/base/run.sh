...
# R CMD BATCH --no-save --no-restore '--args iris 1' main.r out.log

# declare -a arr=("glass") # "iris" "musk")
...
do
	...
	for j in $(seq 1 30);
    do
		R CMD BATCH --no-save --no-restore '--args' --datafile="$i" --epoch="$j" main.r out-"$i"-"$j".log  &
	done
done